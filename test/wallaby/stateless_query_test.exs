defmodule Wallaby.StatelessQueryTest do
  use ExUnit.Case, async: true
  use Quixir

  alias Wallaby.Browser
  alias Wallaby.StatelessQuery
  import StatelessQuery
  ...
  test "the driver can execute queries" do
    ...
    elements =
      session
      |> Browser.visit("/")
      |> Browser.find(StatelessQuery.css("#child"))
  ...
  describe "default count" do
    test "the count defaults to 1 if no count is specified" do
      conditions = StatelessQuery.css(nil).conditions
      assert conditions[:count] == 1

      conditions = StatelessQuery.css(nil, count: 1).conditions
      assert conditions[:count] == 1

      conditions = StatelessQuery.css(nil, count: 3).conditions
      assert conditions[:count] == 3
    end

    test "the count is nil if a minimum or maximum is set" do
      conditions = StatelessQuery.css(nil, minimum: 1).conditions
      assert conditions[:count] == nil
      assert conditions[:minimum] == 1

      conditions = StatelessQuery.css(nil, maximum: 1).conditions
      assert conditions[:count] == nil
  ...
  describe "matches_count?/1" do
    test "the results must match exactly if the count key is specified" do
      query = %StatelessQuery{conditions: [count: 0]}
      assert StatelessQuery.matches_count?(query, 0)

      query = %StatelessQuery{conditions: [count: 1]}
      assert StatelessQuery.matches_count?(query, 1)

      query = %StatelessQuery{conditions: [count: 1]}
      refute StatelessQuery.matches_count?(query, 0)

      query = %StatelessQuery{conditions: [count: 1]}
      refute StatelessQuery.matches_count?(query, 2)
    end

    test "the count key overrides other matching strategies" do
      query = %StatelessQuery{conditions: [count: 1, minimum: 2], result: [%{}]}
      assert StatelessQuery.matches_count?(query, 1)

      query = %StatelessQuery{conditions: [count: 1, minimum: 4, maximum: 2], result: [%{}]}
      assert StatelessQuery.matches_count?(query, 1)
    end

    test "the count must be above the minimum" do
      query = %StatelessQuery{conditions: [minimum: 1], result: [%{}, %{}]}
      assert StatelessQuery.matches_count?(query, 2)

      query = %StatelessQuery{conditions: [minimum: 2], result: [%{}]}
      refute StatelessQuery.matches_count?(query, 1)
    end

    test "the count must be below the maximum" do
      query = %StatelessQuery{conditions: [maximum: 3], result: [%{}, %{}]}
      assert StatelessQuery.matches_count?(query, 2)

      query = %StatelessQuery{conditions: [maximum: 1], result: [%{}, %{}]}
      refute StatelessQuery.matches_count?(query, 2)
    end
    ...
    test "the count must match minimum and maximum filters" do
      ...
      query = %StatelessQuery{conditions: [minimum: 1, maximum: 1], result: [%{}]}
      assert StatelessQuery.matches_count?(query, 1)
    ...
    test "the result is greater then zero if count is any" do
      query = %StatelessQuery{conditions: [count: :any], result: [%{}]}
      assert StatelessQuery.matches_count?(query, 1)
