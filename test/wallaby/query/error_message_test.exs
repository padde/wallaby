defmodule Wallaby.StatelessQuery.ErrorMessageTest do
  ...
  describe "message/1" do
    test "when the results are more then the expected count" do
      message =
        StatelessQuery.css(".test", count: 1)
    ...
    test "when the result is 1" do
      message =
        StatelessQuery.css(".test", count: 3)
        |> Map.put(:result, [1])
        |> ErrorMessage.message(:not_found)
        |> format
    ...
    test "when the result is less then the minimum result" do
      message =
        StatelessQuery.css(".test", minimum: 3, maximum: 5)
        |> Map.put(:result, [1])
        |> ErrorMessage.message(:not_found)
        |> format
    ...
    test "when the result is supposed to be invisible" do
      message =
        StatelessQuery.css(".test", count: 1, visible: false)
        |> Map.put(:result, [1,2,3])
        |> ErrorMessage.message(:not_found)
        |> format
    ...
    test "when the minimum is set less then the maximum" do
      message =
        StatelessQuery.css("", minimum: 6, maximum: 5)
  ...
  describe "visibility/1" do
    test "visible query" do
      query = %StatelessQuery{conditions: [visible: true]}
      assert ErrorMessage.visibility(query) == "visible"
    end

    test "invisible query" do
      query = %StatelessQuery{conditions: [visible: false]}
